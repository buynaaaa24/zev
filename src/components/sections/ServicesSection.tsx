    ...
    >
      ...
      >
        <img
          src={src}
          alt="Service Detail"
          className="w-full h-full object-cover rounded-3xl border-2 border-white/20 drop-shadow-[0_40px_80px_rgba(0,0,0,0.2)]"
        />
      </motion.div>
    </motion.div>
  );
...
export default function ServicesSection({
  ...
  return (
    <section
      id="services"
      ref={sectionRef}
      className="bg-white py-16 lg:py-24 overflow-hidden relative scroll-mt-20"
    >
      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-20 relative z-10">
        ...
        <div
          className={`mb-12 lg:mb-20 flex flex-col items-center sm:items-end sm:flex-row justify-between text-center sm:text-left gap-8 transition-all duration-[800ms] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          <div className="max-w-xl">
            ...
            <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black tracking-tighter text-neutral-900 leading-[0.9] mb-4">
              {services.header.h2Line1 || "Everything"}
              <br />
              <span className="text-neutral-200 italic">
                {services.header.h2Accent || "Simplified"}
              </span>
            </h2>
          </div>
          ...
          <div className="w-full lg:w-[450px] space-y-2 lg:space-y-3 shrink-0 order-2 lg:order-1">
            {features.map((f, i) => (
              ...
              >
                ...
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 ${activeIdx === i ? "bg-black text-white rotate-45 scale-110" : "bg-neutral-100 text-neutral-300 group-hover:bg-neutral-200"}`}
                >
                  <Plus
                    size={20}
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    <span
                      className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${activeIdx === i ? "text-indigo-600" : "text-neutral-200"}`}
                    >
                      {f.id}
                    </span>
                    {activeIdx === i && (
                  ...
                  <h3
                    className={`text-xl sm:text-2xl font-black tracking-tight transition-all duration-500 ${activeIdx === i ? "text-neutral-900 translate-x-1" : "text-neutral-300"}`}
                  >
                    {f.title}
                  </h3>
                </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <InteractiveImage
                  ...
                  accent={features[activeIdx].accent}
                />
