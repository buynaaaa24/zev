...
/* ── Translations (fallback defaults) ───────────────────────────────────────── */
const T = {
  en: {
    hero: {
    ...
    how: {
      ...
      steps: [
        { title: "Driver Registers Free", desc: "Drivers download the ParkEase app and register at no cost. No subscription, no upfront fee — just an account." },
        { title: "Arrive & Scan", desc: "Pull up to the barrier. Scan the entrance QR with the app — or tap the sticker QR on the parking spot if you prefer no app." },
        { title: "Pay & Exit", desc: "Pay via any Mongolian bank, QPay, or card. The system confirms instantly and the exit barrier opens automatically." },
      ],
    },
    payments: {
      label: "Payments",
    ...
    pricing: {
      ...
      mostPopular: "Most Popular", contactUs: "Contact us", custom: "Custom",
      getStarted: "Get Started →",
      note: "Exact pricing based on slot count, location, and requirements.",
      quote: "Request a quote →",
      tiers: [
        { name: "Starter", slots: "Up to 30 slots", features: ["QPay & all bank payments", "Sticker QR at each slot", "Basic occupancy dashboard", "Automatic barrier control", "Free driver registration", "Email support"] },
        { name: "Business", slots: "31 – 100 slots", features: ["Everything in Starter", "Real-time analytics & reports", "Multi-entrance support", "Revenue settlement dashboard", "Custom pricing rules", "Priority support"] },
        { name: "Enterprise", slots: "100+ slots", features: ["Everything in Business", "Multi-location management", "API & system integrations", "Dedicated account manager", "On-site hardware setup", "SLA guarantee"] },
      ],
    },
    free: {
    ...
    cta: { title: ["Ready to automate", "your parking?"], desc: "Tell us about your organization. We'll set up a trial and show you the system in action.", btn: "Request a Trial" },
    footer: { by: "A product by", trial: "Request Trial →" },
  },
  mn: {
    hero: {
    ...
    how: {
      ...
      steps: [
        { title: "Жолооч үнэгүй бүртгүүлнэ", desc: "Жолооч нар ParkEase аппыг татаж аван үнэгүй бүртгүүлнэ. Сарын төлбөр ч, урьдчилсан хураамж ч байхгүй — зөвхөн данс." },
        { title: "Ирж, QR уншуулна", desc: "Хаалт дэргэд зогсож аппаараа QR уншуулна — эсвэл аппгүй бол зогсоолын дээрх стикер QR-ийг уншуулна." },
        { title: "Төлж, гарна", desc: "Монгол банк, QPay эсвэл картаар төлнэ. Систем даруй баталгаажуулж, гарах хаалт автоматаар нээгдэнэ." },
      ],
    },
    payments: {
      label: "Төлбөр",
      ...
      qpayTitle: "QPay", qpayBadge: "Хамгийн их хэрэглэгддэг",
      qpayDesc: "Монгол банк бүр хүлээн зөвшөөрдөг нийтлэг QR төлбөр. Нэг удаа уншуулахад даруй баталгаажна.",
      stickerTitle: "Стикер QR", stickerBadge: "Апп шаардлагагүй",
    ...
    pricing: {
      ...
      mostPopular: "Хамгийн алдартай", contactUs: "Холбоо барих", custom: "Тусгай",
      getStarted: "Эхлэх →",
      note: "Яг үнэ нь зогсоолын тоо, байршил, шаардлагад тулгуурлана.",
      quote: "Үнийн санал авах →",
      tiers: [
        { name: "Starter", slots: "30 хүртэл зогсоол", features: ["QPay болон бүх банкны төлбөр", "Зогсоол бүрт стикер QR", "Үндсэн эзэлхүүний самбар", "Автомат хаалтны удирдлага", "Жолоочийн үнэгүй бүртгэл", "И-мэйл дэмжлэг"] },
        { name: "Business", slots: "31 – 100 зогсоол", features: ["Starter-ийн бүгд", "Бодит цагийн аналитик & тайлан", "Олон оролтын дэмжлэг", "Орлогын тооцооны самбар", "Захиалгат үнийн дүрэм", "Тэргүүлэх дэмжлэг"] },
        { name: "Enterprise", slots: "100+ зогсоол", features: ["Business-ийн бүгд", "Олон байршлын удирдлага", "API & системийн интеграц", "Хувийн менежер", "Газар дээрх тоног суурилуулалт", "SLA баталгаа"] },
      ],
    },
    free: {
    ...
    cta: { title: ["Зогсоолоо автоматжуулах", "бэлэн үү?"], desc: "Байгууллага болон зогсоолынхоо тухай бидэнд хэлнэ үү. Туршилтыг тохируулж, системийг практикаар үзүүлнэ.", btn: "Турших хүсэлт" },
    footer: { by: "Бүтээгч:", trial: "Турших хүсэлт →" },
  },
} as const;
...
function HeroSection() {
  ...
  const defaults = T[lang].hero;
  const api = useContext(AdminCtx)[lang].hero;

  const eyebrow = api.eyebrow || defaults.eyebrow;
  const title1  = api.title1  || defaults.title1;
  const desc    = api.desc    || defaults.desc;
  const cta1    = api.cta1    || defaults.cta1;
  const cta2    = api.cta2    || defaults.cta2;
  const stats   = api.stats?.length ? api.stats : defaults.stats;
  ...
  useEffect(() => { setWordIdx(0); setWordVisible(true); }, [lang]);
  useEffect(() => {
    ...
    <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="hero-content relative z-10 w-full max-w-[1200px] mx-auto px-5 sm:px-10 lg:px-16 pt-28 pb-16 sm:pt-36 sm:pb-24 lg:pt-44">
        ...
        <h1 className={`font-black tracking-tight leading-[1.06] mb-6 sm:mb-8 transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ transitionDelay: ".15s" }}>
          <span className="block text-white text-[36px] sm:text-5xl md:text-6xl lg:text-[84px]">{title1}</span>
          <span
            ...
            {t.steps.map((step, i) => (
              <div key={i} className={`relative transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`} style={{ transitionDelay: visible ? `${i * 120}ms` : "0ms" }}>
                ...
                <div className="relative bg-neutral-50 rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-neutral-100">
                  ...
                  <div className="w-11 h-11 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6" style={{ background: `${YELLOW_GLOW}0.12)`, color: YELLOW_DARK }}>{icons[i]}</div>
...
function FeaturesSection() {
  const { lang } = useParkEaseLang();
  const defaults = T[lang].features;
  const api = useContext(AdminCtx)[lang].features;

  const label = defaults.label;
  const title = defaults.title;
  const desc  = api.desc || defaults.desc;
  const items = api.items?.length ? api.items : defaults.items;
  ...
  return (
    <section id="features" className="bg-neutral-50 py-14 sm:py-24 lg:py-32">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-10 lg:px-16">
        <div ref={ref}>
          <div className={`text-center mb-10 sm:mb-14 lg:mb-20 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <p className="text-sm font-semibold uppercase tracking-widest mb-3" style={{ color: YELLOW_DARK }}>{label}</p>
            <h2 className="text-[28px] sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-neutral-900 mb-4 sm:mb-6">
              {title[0]}<br />{title[1]}
            </h2>
            <p className="text-neutral-400 text-base sm:text-lg max-w-xl mx-auto leading-relaxed font-light">{desc}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {items.map((f, i) => (
              <div key={i} className={`bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-7 border border-neutral-100 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`} style={{ transitionDelay: visible ? `${i * 80}ms` : "0ms" }}>
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center mb-4 sm:mb-5" style={{ background: `${YELLOW_GLOW}0.1)`, color: YELLOW_DARK }}>{icons[i % icons.length]}</div>
...
function PricingSection() {
  const { lang } = useParkEaseLang();
  const defaults = T[lang].pricing;
  const api = useContext(AdminCtx)[lang].pricing;

  const label      = defaults.label;
  const title      = api.title || defaults.title;
  const desc       = api.desc  || defaults.desc;
  const tiers      = api.tiers?.length ? api.tiers : defaults.tiers;
  const { ref, visible } = useReveal();

  const highlight = [false, true, false];
  const prices = [defaults.contactUs, defaults.contactUs, defaults.custom];
    ...
    <section id="pricing" className="bg-white py-14 sm:py-24 lg:py-32">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-10 lg:px-16">
        <div ref={ref}>
          <div className={`text-center mb-10 sm:mb-14 lg:mb-20 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <p className="text-sm font-semibold uppercase tracking-widest mb-3" style={{ color: YELLOW_DARK }}>{label}</p>
            <h2 className="text-[28px] sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-neutral-900 mb-4 sm:mb-6">{title}</h2>
            <p className="text-neutral-400 text-base sm:text-lg max-w-lg mx-auto leading-relaxed font-light">{desc}</p>
          </div>
              ...
              >
                {highlight[i] && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="text-[11px] font-bold px-4 py-1.5 rounded-full uppercase tracking-wide" style={{ background: YELLOW, color: "#1a0f00", boxShadow: `0 4px 14px ${YELLOW_GLOW}0.4)` }}>{defaults.mostPopular}</span>
                  </div>
                )}
                <div className="mb-5 sm:mb-6">
                  ...
                  <p className={`text-xl sm:text-2xl font-black ${highlight[i] ? "text-white" : "text-neutral-900"}`}>{prices[i] ?? defaults.contactUs}</p>
                </div>
          ...
          <p className={`text-center text-neutral-400 text-sm mt-8 sm:mt-10 transition-all duration-700 ${visible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: "0.35s" }}>
            {defaults.note}{" "}
            <Link href="#kholbooBarikh" className="font-medium hover:underline" style={{ color: YELLOW_DARK }}>{defaults.quote}</Link>
          </p>
        </div>
      </div>
    </section>
